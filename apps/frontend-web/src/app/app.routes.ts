...
import { domainTenantGuard } from './core/guards/domain-tenant.guard';

export const routes: Routes = [
  // ← הוסף את זה לגמרי בהתחלה, לפני הכל
  {
    path: '',
    canActivate: [domainTenantGuard],
    children: [
      {
        path: 'auth',
      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            path: 'services',
            loadChildren: () =>
              import('./modules/services/services.routes').then((m) => m.SERVICES_ROUTES),
          },
          {
            path: 'customers',
            loadChildren: () =>
